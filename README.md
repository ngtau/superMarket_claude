...
## 分类管理 + 富文本商品详情(图片+源码) + 支付方式管理 + 默认会员等级 —— 全部端到端验证通过

**1. 分类管理**：后台商品页新增"分類管理"Tab，支持父子分类结构、启停切换、编辑、删除。实测建了"家居用品"→"廚房"两级分类，正确返回`parentId`关联。

**2. 商品详情富文本编辑**：TipTap内核，支持粗体/斜体/标题/列表、图片插入（复用已有上传接口）、"源代碼"模式可直接編輯/粘貼HTML。**顺带修复两个此前从未真正测试过的缺口**：
- 商品编辑功能此前完全不存在（只有新增+上下架，没有"编辑"入口），现补上，支持修改名称/分类/价格/详情/图片。
- **商品描述此前一直被当纯文本渲染**（`<p>{product.description}</p>`），意味着就算之前手动塞HTML进去也不会被解析成格式，只会显示成一堆尖括号文字。现改为`dangerouslySetInnerHTML`+`DOMPurify`消毒渲染。**用独立jsdom环境真实验证了两种XSS向量**：`<script>alert()</script>`标签被完整移除、`<img onerror="alert(1)">`属性注入也被移除，同时`<h2>`/`<strong>`/`<img>`等正常标签完整保留。

**3. 支付方式管理**：后台新增页面，4个渠道（银行转账/FPS/PayMe/AlipayHK）独立开关+专属配置字段。**修复了一个此前从未接通的真实链路**：银行转账收款信息此前一直是后端硬编码的"TODO: 从後台配置讀取"占位符，从Phase5建成到现在从未真正对接过`payment_methods`表。现`BankTransferProvider`会读取并解密真实配置。**端到端实测**：后台设置汇丰银行账户信息 → 客户下单选银行转账 → 付款页`GET /payments/:orderId`拿到的`bankInfo`与后台配置完全一致，不再是占位符。敏感字段（含key/secret/token的字段名）查看时自动脱敏，银行账户信息因本就要展示给客户而原样返回。

**4. 默认会员等级**：种子数据补充"普通/白銀/黃金/鉑金"四个默认等级（此前会员等级列表一直是空的，需要管理员从零手建）。
